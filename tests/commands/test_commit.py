...
def test_commit_previous_file_previous_month(cli, config, tmpdir):
    entries_dir = tmpdir.mkdir('new_entries')
    config.set('default', 'file',
               os.path.join(str(entries_dir), '%m_%Y.txt'))

    entries_dir.join(expand_date('%m_%Y.txt', datetime.date(2014, 1, 1))).write("""01/01/2014
alias_1 2 january
""")

    entries_dir.join(expand_date('%m_%Y.txt', datetime.date(2014, 2, 1))).write("""01/02/2014
alias_1 4 february
""")
